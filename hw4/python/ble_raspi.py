...
class MyDelegate(DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data):
        if cHandle == 19:
            ...
            print(
                f"Magnometer Notification: {ia[0]} {ia[1]} {ia[2]} from Handle {cHandle}"
            )
        else:
            i = int.from_bytes(data, "big")
            print(f"Heart Rate Notification: {i} from Handle {cHandle}")
...
try:
	hrservice = dev.getServiceByUUID(UUID(0x180D))
	for ch in hrservice.getCharacteristics():
		print(str(ch))
